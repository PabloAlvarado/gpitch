...
class LooGP3(gpflow.model.Model):
    def __init__(self, X, Y, kf, kg, Z, whiten=True, minibatch_size=None,
        ...
        q_sqrt_a1 = np.array([0.2*np.eye(self.num_inducing_a1) for _ in range(1)]).swapaxes(0, 2)
        q_sqrt_c1 = np.array([0.2*np.eye(self.num_inducing_c1) for _ in range(1)]).swapaxes(0, 2)
        q_sqrt_a2 = np.array([0.2*np.eye(self.num_inducing_a2) for _ in range(1)]).swapaxes(0, 2)
        q_sqrt_c2 = np.array([0.2*np.eye(self.num_inducing_c2) for _ in range(1)]).swapaxes(0, 2)
