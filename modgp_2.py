...
class Modgp2(gpflow.model.Model):
    def __init__(self, x, y, za, zc, kern, whiten=True, minibatch_size=None):
        ...
        self.q_mu_act = gpflow.param.Param(-2.1972*np.ones((self.za.shape[0], 1)))
        ...
        self.q_sqrt_act = gpflow.param.Param(np.array([np.eye(self.num_inducing_a) for _ in range(1)]).swapaxes(0, 2))
    ...
    def build_prior_kl(self):
        ...
        if self.whiten:
            ...
            k1 = self.kern_com.K(self.zc) + tf.eye(self.num_inducing_c, dtype=float_type) * settings.numerics.jitter_level
            k2 = self.kern_act.K(self.za) + tf.eye(self.num_inducing_a, dtype=float_type) * settings.numerics.jitter_level
    ...
    def predict_com(self, xnew):
        return gpflow.conditionals.conditional(xnew, self.zc, self.kern_com,
                                               self.q_mu_com, q_sqrt=self.q_sqrt_com,
                                               full_cov=False, whiten=self.whiten)

    @gpflow.param.AutoFlow((tf.float64, [None, None]))
    def predict_act(self, xnew):
        return gpflow.conditionals.conditional(xnew, self.za, self.kern_act,
