...
m = gpitch.modpdet.ModPDet(x=x, y=y, fs=fs, ws=N, jump=20) # pitch detection model
m.model.kern1 = kper
m.model.kern2 = kenv
...
fig_array[2, 0].plot(x[::7], gpitch.amtgp.logistic(g[::7]), '.k', mew=0.5)
fig_array[2, 1].plot(x[::7], gpitch.amtgp.logistic(g[::7]), '.k', mew=0.5)
#plt.savefig('../../results/figures/demo_modgp_toy.pdf')
plt.savefig('../../results/figures/demo_modgp_toy.png')
